package hu.qwaevisz.lottery.webservice.filter;
...
@WebFilter(filterName = "LotteryCrossOriginRequestFilter", urlPatterns = { "/*" })
public class LotteryCrossOriginRequestFilter implements Filter {

	private static final Logger LOGGER = Logger.getLogger(LotteryCrossOriginRequestFilter.class.getName());
	...
	public void doFilter(final ServletRequest servletRequest, final ServletResponse servletResponse, final FilterChain chain)
		...
		if (servletRequest instanceof HttpServletRequest) {
			...
			if (LOGGER.isLoggable(Level.FINER)) {
				LOGGER.finer("Request " + method + " url: '" + url + "', query: '" + queryString + "', contentType: " + contentType + "");
			}
		} else {
			if (LOGGER.isLoggable(Level.FINER)) {
				LOGGER.finer("This is not HttpServletRequest (contentType: " + contentType + ").");
			}
		}
