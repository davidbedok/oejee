...
public class BookFacadeImpl implements BookFacade {
	...
	private static final Logger LOGGER = Logger.getLogger(BookFacadeImpl.class.getName());
	...
	public BookStub getBook(String isbn) throws FacadeException {
		try {
			...
			if ( LOGGER.isLoggable(Level.FINE)) {
				LOGGER.fine("Get Book by isbn (" + isbn + ") --> " + stub);
			}
			return stub;
	...
	public List<BookStub> getBooks(BookCriteria criteria) throws FacadeException {
		...
		try {
			...
			if ( LOGGER.isLoggable(Level.FINE)) {
				LOGGER.fine("Get Books by criteria (" + criteria + ") --> " + stubs.size() + " book(s)");
			}
		} catch (final PersistenceServiceException e) {
			LOGGER.log(Level.SEVERE, e.getMessage(), e);
			throw new FacadeException(e.getLocalizedMessage());
